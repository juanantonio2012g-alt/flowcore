#!/usr/bin/env node
/**
 * QA Estructural: Flujo completo del caso - Versión corregida con esquemas reales
 */
...
console.log('║                     (Versión corregida con esquemas reales)                 ║');
console.log('╚══════════════════════════════════════════════════════════════════════════════╝\n');
...
async function fase2Informe() {
  try {
    ...
    // Usar columnas reales de informes_tecnicos
    const informeData = {
      caso_id: casoId,
      resumen_tecnico: 'Informe técnico de QA - análisis preliminar completado',
      hallazgos_principales: 'Se requiere diagnóstico detallado',
      estado_revision: 'completado',
      revisado_por: null  // UUID esperado, usar null por ahora
    };
...
async function fase3Diagnostico() {
  try {
    ...
    // Usar columnas reales de diagnosticos
    const diagnosticoData = {
      caso_id: casoId,
      problematica_identificada: 'Diagnóstico QA - análisis completo realizado',
      ...
      producto_recomendado: 'Servicio completo',
    ...
    const { data: diagVerificado, error: errorVerif } = await supabase
      .from('diagnosticos')
      .select('id, caso_id, problematica_identificada, categoria_caso')
  ...
  try {
    ...
    const { data: validacionActualizada, error: errorValid } = await supabase
      ...
      .update({
        ...
        validado_por: null,  // UUID esperado
        fecha_validacion: new Date().toISOString()
      })
      .eq('caso_id', casoId)
      .select('id, resultado_validacion, validado_por, fecha_validacion')
      .single();
    ...
    console.log(`   ✅ Validador: ${validacionActualizada.validado_por}`);

    console.log('2. Verificando transición workflow...');

    // Verificar transición en workflow_transitions (usando transition_code)
    const { data: transiciones, error: errorTrans } = await supabase
      .from('workflow_transitions')
      .select('id, caso_id, transition_code, from_stage, to_stage, actor')
      .eq('caso_id', casoId)
    ...
    if (errorTrans) {
      ...
      console.log(`   ✅ Código: ${transiciones[0].transition_code}`);
      console.log(`   ✅ Actor: ${transiciones[0].actor}`);
    } else {
...
async function fase5Cotizacion() {
  try {
    ...
    // Usar columnas reales de cotizaciones
    const cotizacionData = {
      caso_id: casoId,
      solucion_asociada: 'Cotización QA - servicios completos',
      productos_incluidos: 'Servicio técnico completo',
      cantidades: '1 unidad',
      condiciones: '50% anticipo, 50% entrega',
      observaciones: 'Cotización generada por QA',
      monto: 150000,
      estado: 'emitida'
    };
    ...
    const { data: transiciones, error: errorTrans } = await supabase
      .from('workflow_transitions')
      .select('id, caso_id, transition_code, from_stage, to_stage')
      .eq('caso_id', casoId)
...
async function fase6Aprobacion() {
  try {
    ...
    console.log('1. Actualizando caso con aprobación...');

    // Actualizar el caso directamente (simulando seguimiento con aprobación)
    const { data: casoActualizado, error: errorCaso } = await supabase
      ...
      .update({
        ...
        seguimiento_por: null  // UUID esperado
      })
      .eq('id', casoId)
      .select('id, estado_comercial, proxima_accion, proxima_fecha')
      .single();
    ...
    console.log(`   ✅ Caso actualizado: ${casoActualizado.id}`);
    console.log(`   ✅ Estado comercial: ${casoActualizado.estado_comercial}`);
    console.log(`   ✅ Próxima acción: ${casoActualizado.proxima_accion}`);
    ...
    const { data: transiciones, error: errorTrans } = await supabase
      ...
      .select('id, caso_id, transition_code, from_stage, to_stage, actor')
      .eq('caso_id', casoId)
      .eq('transition_code', 'cliente_aprobo')
      .order('occurred_at', { ascending: false })
      .limit(1);
    ...
    const { data: casoFinal, error: errorCasoFinal } = await supabase
      .from('casos')
    ...
    if (errorCasoFinal) {
      throw new Error(`Error verificando caso final: ${errorCasoFinal.message}`);
    }

    console.log(`   ✅ Estado del caso: ${casoFinal.estado}`);
    ...
    if (casoFinal.estado === 'logistica_entrega') {
      console.log('   ✅ Caso cambió correctamente a logistica_entrega');
    } else {
      console.log(`   ⚠️  Caso aún en estado: ${casoFinal.estado}`);
...
async function fase7ValidacionFinal() {
  try {
    ...
    if (errorLog) {
      ...
      console.log('   ℹ️  No hay logística registrada aún (esperado - se registra desde UI)');
    }
...
async function ejecutarQA() {
  try {
    ...
    console.log('   Flujo: solicitud → informe → diagnóstico → validación → cotización → aprobación');

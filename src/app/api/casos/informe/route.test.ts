...
describe("POST /api/casos/informe", () => {
  const originalNodeEnv = process.env.NODE_ENV;

  beforeEach(() => {
    vi.clearAllMocks();
    process.env.NODE_ENV = originalNodeEnv;
    executeInforme.mockResolvedValue({
      ok: true,
      caso_id: "caso-1",
    ...
    buildInformeEvent.mockReturnValue(null);
  ...
  it("usa service role en development cuando no hay bearer token", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/casos/informe", {
        method: "POST",
        headers: {
    ...
    expect(executeInforme).toHaveBeenCalledWith(
